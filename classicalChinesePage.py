...
# 创建蓝图
classical_chinese_page = Blueprint(
    "classical_chinese_page",
    __name__,
...
# 页面：文言文分析主页面
@classical_chinese_page.route("/classicalChinese")
def index():
    return render_template("classicalChinese.html")


# 接口：逐句翻译
@classical_chinese_page.route("/api/translate", methods=["POST"])
def translate():
    try:
...
# 接口：重点字词解释
@classical_chinese_page.route("/api/explain-words", methods=["POST"])
def explain_words():
    try:
        data = request.get_json()
        content = data.get("content")
...
@classical_chinese_page.route("/api/analyze-theme", methods=["POST"])
def analyze_theme():
    try:
        data = request.get_json()
        content = data.get("content")
